...
const HolidayScreen = ({ navigation, route }) => {
  ...
  function backArrow() {
    return (
      ...
      >
        ...
        <Text style={{ ...Fonts.whiteColor20Regular }}>Holiday</Text>
      </View>
    );
  }
};
    ...
    >
      ...
      {item.showNotice && (
        <View style={styles.noticeDetails}>
          ...
          <Text style={styles.noticeDate}>Start Date: {item.start_date}</Text>
          <Text style={styles.noticeDate}>End Date: {item.end_date}</Text>
          <Text style={styles.noticeDate}>Message: {item.message}</Text>
        </View>
      )}
    </TouchableOpacity>
  );
